    ...
    <div className={preferences.motion_mode === "reduced" ? "motion-reduced" : ""}>
      ...
      <div className="flex min-h-dvh">
        ...
        <div className="flex min-w-0 flex-1 flex-col pb-24 md:pb-0">
          <header className="flex items-center justify-between border-b border-[var(--border-subtle)] px-4 py-4 md:px-8">
            <AssistantOrb state="ready" reducedMotion={preferences.motion_mode === "reduced"} />
            <form action={logoutAction}>
              <button className="touch-target rounded-xl border border-[var(--border-subtle)] px-4 text-sm text-[var(--text-secondary)]">
                Log out
              </button>
            </form>
          </header>
          <div className="grid min-h-0 flex-1 grid-cols-1 xl:grid-cols-[minmax(0,1fr)_340px]">
            ...
            >
              <h2 className="text-sm font-semibold text-[var(--text-secondary)]">Context inspector</h2>
              <p className="mt-3 text-sm leading-6 text-[var(--text-muted)]">
                Source inspection will appear here after document, retrieval, and conversation milestones.
              </p>
            </aside>
          </div>
        </div>
